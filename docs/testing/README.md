...
6반 4조의 팀원과 평가자가 같은 입력으로 동작을 재현하고 결과를 확인하기 위한 안내서다. 현재 기능, 실행 명령, 테스트별 입력·기대 결과·확인 방법을 제공한다. 자동 회귀는 통과했으며 실제 모델의 대화 품질과 실계정 API 동작은 별도 시연으로 확인한다.

## 1 현재 구성과 검증 범위

기준 main: `2bc2ce63562a9bd17b1a3509e30cf3e83330df0f`. 후속 변경: `feat/a-runtime-hardening`. 실제 실행 SHA와 작업 트리 변경 유무는 실행 결과 JSON에 기록한다.
...
| ID | 입력과 실행 | 기대 결과와 기록 |
|---|---|---|
| M01 | 30분 안에 식사, 체류 12분 요청 후 표시된 후보 선택 | 마감 14:25와 왕복·체류 시간. 선택 후 추가 승인 없이 현재 상태 재검증·확정 |
