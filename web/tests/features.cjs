...
(async () => {
  ...
  try {
    ...
    await page.locator('[name=target]').fill('90'); await page.locator('[name=limit]').fill('25'); await page.locator('[name=dwell]').fill('10');
    ...
    assert.match(requests.at(-1).body.text, /90%.*25분.*10분/);
